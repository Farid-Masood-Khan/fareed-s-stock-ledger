...
const ShopkeepersPage = () => {
  ...
  const shopkeeperTransactions = selectedShopkeeper ? transactions.filter(t => t.shopkeeperId === selectedShopkeeper.id).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) : [];
  return <div className="space-y-6 my-[28px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <ShopkeeperList shopkeepers={shopkeepers} selectedShopkeeper={selectedShopkeeper} searchTerm={searchTerm} onSearchChange={setSearchTerm} onSelectShopkeeper={setSelectedShopkeeper} />

        {selectedShopkeeper ? <ShopkeeperDetails selectedShopkeeper={selectedShopkeeper} transactions={shopkeeperTransactions} newPayment={newPayment} onNewPaymentChange={handleNewPaymentChange} onAddPayment={handleAddPayment} /> : <div className="lg:col-span-2">
        ...
        <AddShopkeeperDialog isOpen={isAddDialogOpen} onOpenChange={setIsAddDialogOpen} newShopkeeper={newShopkeeper} onNewShopkeeperChange={handleNewShopkeeperChange} onAddShopkeeper={handleAddShopkeeper} />
      </div>
    </div>;
};
