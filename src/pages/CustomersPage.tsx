...
const CustomersPage = () => {
  ...
  const filteredCustomers = customers?.filter(customer => customer.name.toLowerCase().includes(searchTerm.toLowerCase()) || customer.contact && customer.contact.toLowerCase().includes(searchTerm.toLowerCase()) || customer.cnic && customer.cnic.toLowerCase().includes(searchTerm.toLowerCase()) || customer.email && customer.email.toLowerCase().includes(searchTerm.toLowerCase())) || [];

  // Sort by most recent first
  const sortedCustomers = [...filteredCustomers].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  const customerSales = selectedCustomer ? sales.filter(sale => sale.customerId === selectedCustomer.id).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) : [];
  const handleAddCustomer = () => {
    ...
    if (newCustomer.contact || newCustomer.cnic) {
      const existingCustomer = customers?.find(c => newCustomer.contact && c.contact === newCustomer.contact || newCustomer.cnic && c.cnic === newCustomer.cnic);
      if (existingCustomer) {
        toast({
  ...
  const handleEditCustomer = () => {
    ...
    if (newCustomer.contact || newCustomer.cnic) {
      const existingCustomer = customers?.find(c => c.id !== selectedCustomer.id && (newCustomer.contact && c.contact === newCustomer.contact || newCustomer.cnic && c.cnic === newCustomer.cnic));
      if (existingCustomer) {
        toast({
  ...
  const calculateTotalSpent = (customerId: string): number => {
    return sales.filter(sale => sale.customerId === customerId).reduce((total, sale) => total + sale.total, 0);
  };
  return <div className="space-y-6 my-[28px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            {paginatedCustomers.length === 0 ? <div className="text-center py-8">
                ...
                <div className="space-y-2">
                  {paginatedCustomers.map(customer => <div key={customer.id} className={`flex items-center justify-between p-3 rounded-md border cursor-pointer transition-colors ${selectedCustomer?.id === customer.id ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`} onClick={() => setSelectedCustomer(customer)}>
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="ghost" size="icon" className={`h-8 w-8 ${selectedCustomer?.id === customer.id ? "text-primary-foreground hover:text-primary-foreground/80" : ""}`} onClick={e => {
                    e.stopPropagation();
                    openEditDialog(customer);
                  }}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className={`h-8 w-8 ${selectedCustomer?.id === customer.id ? "text-primary-foreground hover:text-primary-foreground/80" : "hover:text-destructive"}`} onClick={e => {
                    e.stopPropagation();
                    setSelectedCustomer(customer);
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {selectedCustomer ? <Tabs defaultValue="details">
                        ...
                        <div className="space-y-2 rounded-md border p-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium">
                              {customerSales.length > 0 ? formatDateTime(customerSales[0].date) : "No purchases yet"}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <TabsContent value="purchases">
                  <div className="space-y-4">
                    ...
                    {customerSales.length === 0 ? <div className="text-center py-8 border rounded-md">
                        ...
                        <table className="w-full border-collapse">
                          ...
                          <tbody>
                            {customerSales.map(sale => <tr key={sale.id} className="border-t hover:bg-muted/50">
                                <td className="p-3">
                                ...
                                <td className="p-3">
                                  <div className="flex flex-col gap-1">
                                    {sale.items.slice(0, 2).map((item, i) => <div key={i} className="text-sm">
                                        {item.productName} x{item.quantity}
                                      </div>)}
                                    {sale.items.length > 2 && <div className="text-xs text-muted-foreground">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="Customer name" value={newCustomer.name} onChange={e => setNewCustomer({
              ...newCustomer,
              name: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="contact" placeholder="Phone number" value={newCustomer.contact} onChange={e => setNewCustomer({
              ...newCustomer,
            ...
            <div className="space-y-2">
              ...
              <Input id="cnic" placeholder="CNIC number" value={newCustomer.cnic} onChange={e => setNewCustomer({
              ...newCustomer,
              cnic: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="customer@example.com" value={newCustomer.email} onChange={e => setNewCustomer({
              ...newCustomer,
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="edit-name" placeholder="Customer name" value={newCustomer.name} onChange={e => setNewCustomer({
              ...newCustomer,
              name: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="edit-contact" placeholder="Phone number" value={newCustomer.contact} onChange={e => setNewCustomer({
              ...newCustomer,
              contact: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="edit-cnic" placeholder="CNIC number" value={newCustomer.cnic} onChange={e => setNewCustomer({
              ...newCustomer,
              cnic: e.target.value
            })} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="edit-email" type="email" placeholder="customer@example.com" value={newCustomer.email} onChange={e => setNewCustomer({
              ...newCustomer,
              email: e.target.value
            })} />
            </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            ...
            {customerSales.length > 0 ? <span className="text-red-500 block mt-2 font-semibold">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteCustomer} disabled={customerSales.length > 0}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
